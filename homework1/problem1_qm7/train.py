	...
	for i in tqdm(range(1,100000)):
		...
		if augment:
			...
			for j in range(mb):
				...
				#I realize that this now takes radians as argument, however, mathematically it shouldn't matter (even random dist for a sinusoidal fn)
				R_aug[j] = rotate_3d_coordinates(R[r][j], x_angle, y_angle, z_angle)
			R_batch = R_aug
		else:
			R_batch = R[r]
		E_pred = nnsgd.forward((R_batch, Z[r]))
...
if __name__ == '__main__':
	...
	train(nnsgd, nnavg, R, Z, E_true, args.mb, args.hist, augment= True)
