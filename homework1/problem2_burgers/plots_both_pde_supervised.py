...
model = torch.load('model_both_losses.pth', map_location=torch.device('cpu'))
model.eval()
...
training_both = np.load(f'training-both_losses.npy')
plt.figure()
plt.plot(np.linspace(0,len(training_both),len(training_both)-1),training_both[1:])
plt.show()

training_rmses_both = np.load(f'training-rmses_both_losses.npy')
print(training_rmses_both)
