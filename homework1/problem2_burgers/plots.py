...
training_losses = np.load("training-losses.npy")
...
PDE_resid_loss = np.load(f'pde_resid_loss.npy')
plt.figure()
print(f'Mean of:',np.mean(PDE_resid_loss))
print(f'St. Dev of:',np.std(PDE_resid_loss))

plt.plot(np.linspace(1,len(PDE_resid_loss),40),PDE_resid_loss)
plt.show(block=False)

dataloss = np.load(f'dataloss-alpha1.npy')
pdeloss = np.load(f'pdeloss-alpha1.npy')
print(dataloss)
plt.figure()
plt.plot(np.arange(0,40), dataloss )
plt.title('Data Loss for alpha =1 ')
plt.show(block=False)

plt.figure()
plt.plot(np.arange(0,40), pdeloss )
plt.title('PDE Residual Loss for alpha =1 ')
plt.show()
