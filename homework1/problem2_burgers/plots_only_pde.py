...
training_losses = np.load("training-pde_only_losses.npy")
#model = torch.load('model.pth')
model = torch.load('model_pde_only_losses.pth', map_location=torch.device('cpu'))
model.eval()
...
ax1.set_title('Predicted Solution PDE only')
...
training_pde = np.load(f'training-pde_only_losses.npy')
plt.figure()
plt.plot(np.linspace(0,len(training_pde),len(training_pde)-1),training_pde[1:])
plt.show()